class DeliveriesController < ApplicationController
  ...
  def new
    ...
    @loans = []
    Loan.all.each do | l |
      delivery_amount = Delivery.where(:loan_id => l.id).sum(:amount)
      if l.amount.to_f > delivery_amount.to_f
        @loans << l
      end
    end
  end
  ...
  def create
    ...
    loan = Loan.find(@delivery.loan_id)
    total_amount = Delivery.where(:loan_id => loan.id).sum(:amount)

    if loan.amount < (total_amount + @delivery.amount)
