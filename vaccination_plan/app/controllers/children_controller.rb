class ChildrenController < ApplicationController
  ...
  def index
    @children = Child.all
    @children = @children.where(:sex => params[:gender]) unless params[:gender].blank?
    @children = @children.where(["first_name LIKE :filter OR last_name LIKE :filter", :filter => "%" + params[:filter] + "%"]) unless params[:filter].blank?
    @children = @children.order((params[:by] || "first_name") + " " + (params[:order] || "asc"))
