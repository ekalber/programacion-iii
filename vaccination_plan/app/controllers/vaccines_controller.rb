class VaccinesController < ApplicationController
  ...
  def index
    ...
    @vaccines = @vaccines.where(["name LIKE :filter", :filter => "%" + params[:filter] + "%"]) unless params[:filter].blank?
  end
  ...
  def create
    ...
    respond_to do |format|
      if @vaccine.save
        format.html { redirect_to vaccines_url, notice: "Vaccine was successfully created." }
